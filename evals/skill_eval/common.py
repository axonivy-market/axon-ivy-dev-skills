"""Shared infrastructure: tunables, path resolution, JSON IO, filesystem helpers.

Everything here is provider-agnostic and free of business logic — the pieces the
other modules would otherwise each re-implement.
"""
...
# Directories never copied into a workspace snapshot or scanned for changes.
DEFAULT_EXCLUDES = {
    ".git", "target", "src_generated", "node_modules", ".idea", ".settings",
}

# Max bytes of a single changed file, and total across all files, embedded in
# the grading prompt. Keeps us well under ARG_MAX and the model context window.
MAX_FILE_BYTES = 16_000
MAX_BUNDLE_BYTES = 120_000

# Per-run subprocess timeout (seconds).
DEFAULT_TIMEOUT = 900
...
def resolve_skill_dir(repo_root, skill):
    """A skill given as a path is used as-is; a bare name resolves under repo root."""
