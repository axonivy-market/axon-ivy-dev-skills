...
def _build_prompt(case, work_dir, *, with_skill, skill_dir, invoke_skill):
    prompt = case.get("prompt", "")

    force_skill = invoke_skill and case.get("should_trigger") is None
    if with_skill and force_skill:
...
def _trigger_assertions(case, *, with_skill, usage, skill_dir):
    if not with_skill or case.get("should_trigger") is None:
        return []
    expected = bool(case["should_trigger"])
    fired = bool(usage and skill_dir.name in (usage.invoked_skills or []))
    return [{
        "text": (f"Skill '{skill_dir.name}' triggers naturally for this prompt"
                 if expected else
                 f"Skill '{skill_dir.name}' stays quiet for this prompt (negative control)"),
        "passed": fired == expected,
        "evidence": f"invoked_skills={usage.invoked_skills if usage else []}; expected_fire={expected}",
    }]


def run_configuration(case, config_dir, work_dir, *, with_skill, skill_dir, dep_skill_dirs=(),
                      fixture_dir, files, skill_repo_root, invoke_skill, timeout,
    ...
    prompt = _build_prompt(
        case, work_dir, with_skill=with_skill, skill_dir=skill_dir, invoke_skill=invoke_skill)

    load_dirs = [skill_dir, *dep_skill_dirs] if with_skill else []

    print(f"  [{label}] running task")
    task_result = provider.run_task(
        prompt, work_dir, skill_dirs=load_dirs,
    ...
    extra_results = _trigger_assertions(
        case, with_skill=with_skill, usage=task_result.usage, skill_dir=skill_dir)
...
def evaluate(*, skill_dir, cases, fixtures_dir, repo_root, iteration_dir,
    ...
    try:
        return _evaluate_cases(
            ...
            dep_skill_dirs=dep_skill_dirs)
    finally:
        shutil.rmtree(runs_root, ignore_errors=True)


def _evaluate_cases(runs_root, *, skill_dir, cases, fixtures_dir, repo_root,
                    iteration_dir, model, timeout, invoke_skill, provider,
                    dep_skill_dirs=()):
    per_eval = []
    for case in cases:
        eid = case.get("id", "?")
        ...
        trigger_only = case.get("should_trigger") is not None and not case.get("assertions")
        configs = [("with_skill", True)]
        if not trigger_only:
            configs.append(("without_skill", False))
        else:
            print("  [without_skill] skipped — trigger-only case (nothing to grade without the skill)")

        for label, with_skill in configs:
            work_dir = runs_root / f"{eid}-{label}"
            record[label] = run_configuration(
                case, eval_dir / label, work_dir,
                with_skill=with_skill, skill_dir=skill_dir, dep_skill_dirs=dep_skill_dirs,
