...
There is **no** `web.xml` or `@ApplicationPath` bootstrap class in an Ivy IAR project — the engine auto-discovers JAX-RS resources on the classpath. Do not add a `jakarta.ws.rs.core.Application` subclass.
